...
var path = require('path'),
    ...
    commandLineOptions = require('optimist')
        .usage('$0 --i18n <pathToI18nFile> [--locales <localeId>[,<localeId>...]] [--defaultlocale <localeId>] --babeldir=<dirContainingTheBabelFilesToApply> --root <inputRootDirectory> <htmlFile>...')
...
new AssetGraph({root: commandLineOptions.root})
    ...
    .bundleSystemJs({ sourceMaps: true, conditions: { 'locale.js': defaultLocaleId, 'locale': defaultLocaleId } })
    .bundleRequireJs({ sourceMaps: true })
    .populate({startAssets: {type: 'JavaScript'}, followRelations: { to: { url: /^file:\// } } })
    .serializeSourceMaps()
    ...
    .queue(function importLanguageKeys(assetGraph) {
        ...
        Object.keys(translationsByKeyAndLocaleId).forEach(function (key) {
            ...
            Object.keys(translationsByKeyAndLocaleId[key]).forEach(function (localeId) {
                ...
                if (isArrayOrObject(existingTranslation) && isArrayOrObject(newTranslation)) {
                    // Do a 'deep extend' to update existing complex value:
                    setMergedTranslation(_.merge(Array.isArray(newTranslation) ? [] : {}, existingTranslation, newTranslation));
