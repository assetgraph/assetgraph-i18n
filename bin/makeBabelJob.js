...
var path = require('path'),
    ...
    commandLineOptions = require('optimist')
        .usage('$0 --i18n <pathToI18nFile> [--all] [--defaultlocale <localeId>] --babeldir=<dirForBabelFiles> --root <inputRootDirectory> --locales <localeId>,... <htmlFile>...')
        .boolean('all')
        .demand(['root', 'locales', 'babeldir', 'i18n'])
        .argv,
...
new AssetGraph({root: commandLineOptions.root})
    .logEvents({repl: commandLineOptions.repl, stopOnWarning: commandLineOptions.stoponwarning})
    .loadAssets(initialAssetUrls)
    .populate({from: {type: 'Html'}, followRelations: {type: 'HtmlScript', to: {url: /^file:/}}})
    .bundleSystemJs({ conditions: { 'locale.js': defaultLocaleId, 'locale': defaultLocaleId } })
    .bundleRequireJs()
    .serializeSourceMaps()
    .populate({ startAssets: { type: 'SourceMap' }, followRelations: { to: { url: /^file:\// } } })
    .populate({
        followRelations: {type: {$nin: ['HtmlAnchor', 'HtmlMetaRefresh']}, to: {url: AssetGraph.query.not(/^https?:/)}}
    })
    .queue(require('../lib/transforms/checkLanguageKeys')({supportedLocaleIds: localeIds, defaultLocaleId: defaultLocaleId}))
    .queue(function exportLanguageKeys(assetGraph) {
        ...
        localeIds.forEach(function (localeId) {
            ...
            keys.forEach(function (key) {
                ...
                var i18nAssetForKey = assetGraph.findAssets({
                    type: 'I18n',
                    isLoaded: true,
