...
describe('checkLanguageKeys', function () {
    it('should handle a combo test case', function () {
        var infos = [];
        return new AssetGraph({root: __dirname + '/../../testdata/transforms/checkLanguageKeys/combo/'})
            .on('info', function (err) {
                infos.push(err);
            })
            .loadAssets('index.html')
            .populate()
            .queue(require('../../lib/transforms/checkLanguageKeys')({
    ...
    it('a space at the end of a TR original text', function () {
        var infos = [];
        return new AssetGraph({root: __dirname + '/../../testdata/transforms/checkLanguageKeys/neverEndingSpaceLoop/'})
            .on('info', function (err) {
                infos.push(err);
            })
            .loadAssets('index.html')
    ...
    it('warns when a structured value with plural rule keys is non-exhaustive for a given locale', function () {
        var infos = [];
        return new AssetGraph({root: __dirname + '/../../testdata/transforms/checkLanguageKeys/pluralRule/'})
            .on('info', function (err) {
                infos.push(err);
            })
            .loadAssets('index.html')
    ...
    it('emits info events when multiple language keys have the same translation in the default locale', function () {
        var infos = [];
        return new AssetGraph({root: __dirname + '/../../testdata/transforms/checkLanguageKeys/duplicateLanguageKeys/'})
            .on('info', function (err) {
                infos.push(err);
            })
            .loadAssets('index.html')
