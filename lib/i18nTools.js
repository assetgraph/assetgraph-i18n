        ...
        enter: function (node, parentNode) {
            ...
            if (node.type === 'CallExpression' &&
                ...
                if (keyAndDefaultValue) {
                    if (lambda(_.extend(keyAndDefaultValue, {type: 'callTR.PAT', node: node, parentNode: parentNode})) === false) {
                        return this.break();
                    }
                }
            } else if (node.type === 'CallExpression' && node.callee.type === 'Identifier' && node.callee.name === 'TR') {
                keyAndDefaultValue = extractKeyAndDefaultValueFromCallNode(node);
                if (keyAndDefaultValue) {
                    if (lambda(_.extend(keyAndDefaultValue, {type: 'TR', node: node, parentNode: parentNode})) === false) {
                        return this.break();
                    }
                }
                ...
                keyAndDefaultValue = extractKeyAndDefaultValueFromCallNode(node);
                if (keyAndDefaultValue) {
                    if (lambda(_.extend(keyAndDefaultValue, {type: 'TR.PAT', node: node, parentNode: parentNode})) === false) {
                        return this.break();
                    }
                }
            }
    ...
    return function trReplacer(options) {
        ...
        if (type === 'callTR.PAT') {
            ...
            if (valueAst.type !== 'Literal' || typeof valueAst.value !== 'string') {
                console.warn('trReplacer: Invalid TR.PAT syntax: ' + escodegen.generate(node));
                return;
            }
...
i18nTools.findOccurrences = function (assetGraph, initialAssets) {
    ...
    initialAssets.forEach(function (htmlAsset) {
        assetGraph.collectAssetsPostOrder(htmlAsset, {type: {$nin: ['HtmlAnchor', 'HtmlMetaRefresh', 'SvgAnchor']}}).forEach(function (asset) {
