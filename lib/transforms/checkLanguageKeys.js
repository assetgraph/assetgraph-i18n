...
module.exports = function (options) {
  ...
  return function checkLanguageKeys(assetGraph) {
    ...
    Object.keys(whitespaceWarningsByKey).forEach(
      function (whitespaceWarningKey) {
        emitEvent(
          'whitespace',
          new Error(
            'Language key ' +
              ...
              if (
                ...
                values.forEach(function (value, i) {
                  ...
                  if (!_.isEqual(actualKeys, expectedKeys)) {
                    _.difference(actualKeys, expectedKeys).forEach(
                      function (extraKey) {
                        emitEvent(
                          'warn',
                          new Error(
                    ...
                    _.difference(expectedKeys, actualKeys).forEach(
                      function (missingKey) {
                        emitEvent(
                          'warn',
                          new Error(
                ...
                keysPresentInOneOrMoreValues.forEach(
                  function (keyPresentInOneOrMoreValues) {
              ...
              values.forEach(function (value, i) {
                if (value && typeof value === 'object') {
                  emitEvent(
                    'warn',
                    new Error(
                      localeIds[i] + ' has an object at ' + explodedKey,
                    ),
                  );
                }
              });
    ...
    if (defaultLocaleId) {
      ...
      Object.keys(keysByStringifiedDefaultLocaleValue).forEach(
        function (stringifiedDefaultLocaleValue) {
          const keys =
            keysByStringifiedDefaultLocaleValue[stringifiedDefaultLocaleValue];
          if (keys.length > 1) {
