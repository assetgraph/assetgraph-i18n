const i18nTools = require('../i18nTools'), _ = require('lodash'), util = require('util'), AssetGraph = require('assetgraph'), canonicalizeObject = require('../canonicalizeObject');
...
const defaultAttributeNamesThatMustBeTranslated = ['placeholder', 'title', 'alt'];
...
module.exports = function(options) {
  options = options || {};
  const defaultLocaleId = options.defaultLocaleId, supportedLocaleIds = options.supportedLocaleIds, ignoreByMessageType = normalizeToObject(options.ignoreMessageTypes), warnByMessageType = normalizeToObject(options.warnMessageTypes), mustBeTranslatedByAttributeName = {};
  ...
  return function checkLanguageKeys(assetGraph) {
    const localeIdsByMissingKey = {}, localeIdsByUntranslatedKey = {}, defaultValueInfosByKey = {}, allKeysByLocaleId = {}, whitespaceWarningsByKey = {}, isSeenByKey = {}, occurrencesByKey = {}; // FIXME: Will end up containing a duplicate for each locale
            ...
            if (!localeId || !(isDefaultLocaleId && options.defaultValue)) {
              if (typeof value === 'undefined') {
                ...
                if (localeIdsByUntranslatedKey[key]) {
                  if (
                    !localeIdsByUntranslatedKey[key].includes(localeId)
                  ) {
    ...
    assetGraph
      ...
      .forEach(function(i18nAsset) {
        ...
        Object.keys(i18nAsset.parseTree).forEach(function(key) {
          ...
          const valueByLocaleId = allKeys[key], localeIds = Object.keys(valueByLocaleId);
              ...
              if (
                values.every(function(value) {
                  ...
                  return keys.every(function(key) {
                    return ['zero', 'one', 'two', 'few', 'many', 'other'].includes(key);
                  });
                })
              ) {
                values.forEach(function(value, i) {
                  const localeId = localeIds[i],
                        ...
                        expectedKeys = require('plurals-cldr')
                          .forms(localeId)
                          .sort();
    ...
    if (defaultLocaleId) {
      const keysByStringifiedDefaultLocaleValue = {}, defaultValueMismatchInfos = {};
      ...
      .forEach(function(htmlAsset) {
        ...
        Array.prototype.slice
          ...
          .forEach(function(element) {
            ...
            const i18nAttributes = _.toArray(element.attributes).filter(function(
              attribute
            ) {
              const attributeName = attribute.nodeName.toLowerCase(), attributeValue = attribute.nodeValue;
