...
function getParsedDataSystemJsConditionalsFromRelation(relation) {
  if (relation.type === 'HtmlScript' || relation.type === 'HtmlStyle') {
    var dataCond = relation.node.getAttribute('data-assetgraph-conditions');
    if (dataCond) {
...
function assetNeedsLocalization(asset, assetGraph) {
  ...
  if (
    incomingRelations.some(function(incomingRelation) {
      var parsedDataCond = getParsedDataSystemJsConditionalsFromRelation(
        incomingRelation
      );
      return (
        parsedDataCond &&
    ...
    assetGraph
      ...
      .forEach(function(originalHtmlAsset) {
        ...
        localeIds.forEach(function(localeId) {
          var allKeysForLocale = i18nTools.extractAllKeysForLocale(
              assetGraph,
              localeId
            ),
          ...
          function localizeAsset(asset) {
            if (asset.type === 'JavaScript') {
              ...
              cssRules.reverse().forEach(function(node) {
                var matchSelectorText = node.selector.match(
                ...
                if (matchSelectorText) {
                  ...
                  node.selector
                    ...
                    .forEach(function(selectorFragment) {
                      var matchSelectorFragment = selectorFragment.match(
          ...
          nonInlineAssetsToLocalizePreOrder.forEach(function(asset) {
            var incomingRelationsFromLocalizedAssets = assetGraph.findRelations(
              {
                to: asset,
                from: { nonInlineAncestor: { $in: localizedAssets } }
              }
            );
            var targetUrl = asset.url.replace(
              /(\.\w+)?$/,
              '.' + localeId + '$1'
            );
            var existingLocalizedAsset = assetGraph.findAssets({
              url: targetUrl
            })[0];
            var incomingHtmlScriptsAndHtmlStyles = incomingRelationsFromLocalizedAssets.filter(
              function(relation) {
                return (
                  relation.type === 'HtmlScript' ||
            ...
            if (existingLocalizedAsset) {
              ...
              var localizedAsset = asset.clone(
