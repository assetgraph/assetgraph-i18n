        ...
        assetGraph.findAssets(_.extend({type: 'Html'}, queryObj)).forEach(function (originalHtmlAsset) {
            ...
            localeIds.forEach(function (localeId) {
                ...
                nonInlineAssetsToLocalizePreOrder.forEach(function (asset) {
                    var incomingRelationsFromLocalizedAssets = assetGraph.findRelations({to: asset, from: {nonInlineAncestor: {$in: localizedAssets}}});
                        ...
                        if (localeValue) {
                            if (localeValue === localeId) {
                                ...
                                if (remainingKeys.length === 0) {
                                    incomingHtmlScriptsAndHtmlStyles[0].node.removeAttribute('data-assetgraph-conditions');
                                } else {
                                    incomingHtmlScriptsAndHtmlStyles[0].node.setAttribute('data-assetgraph-conditions', remainingKeys.map(function (key) {
